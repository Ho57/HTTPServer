...
httprun(int ssock, char * query_string){
    ...
    if(!name[0]) print_error("you didn't give your name", fssock);
    if(!address[0]) print_error("you didn't give your address", fssock);
    if(!phone[0]) print_error("you didn't give your phone number", fssock);
    if((!subs) && (!slims) && (!sides) && (!drinks)) print_error("you didn't order anything", fssock);
